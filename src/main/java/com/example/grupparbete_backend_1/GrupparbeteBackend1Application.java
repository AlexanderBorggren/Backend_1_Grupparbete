...
/*
    ...
    public CommandLineRunner addDefaultData(CustomerRepo customerRepo, RoomTypeRepo roomTypeRepo, RoomRepo roomRepo, BookingRepo bookingRepo) {
        return (args) -> {
            ...
            RoomType singleRoom = new RoomType("Single", 0, 300, Timestamp.from(Instant.now()), Timestamp.from(Instant.now()));
            RoomType doubleRoom1 = new RoomType("Double room 1", 1, 600, Timestamp.from(Instant.now()), Timestamp.from(Instant.now()));
            RoomType doubleRoom2 = new RoomType("Double room 2", 2, 685, Timestamp.from(Instant.now()), Timestamp.from(Instant.now()));
